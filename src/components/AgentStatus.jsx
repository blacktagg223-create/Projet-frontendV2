...
const AgentCard = ({ agent }) => {
  const stateConfig = AGENT_STATES[agent.status] || AGENT_STATES.IDLE;
      ...
      <div className="flex items-start justify-between mb-3">
        <div>
          <h3 className="font-medium text-white text-sm mb-1">
            {agent.id.replace(/_/g, ' ').toUpperCase()}
          </h3>
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-gray-400">
            {agent.last_action ? formatTimestamp(agent.last_action) : agent.last_cycle ? formatTimestamp(agent.last_cycle) : agent.last_prediction ? formatTimestamp(agent.last_prediction) : agent.last_scan ? formatTimestamp(agent.last_scan) : 'No activity'}
          </span>
        </div>
